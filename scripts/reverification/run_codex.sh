...
# subscription auth). Mirrors run_opus.sh in structure: per-uid batch -> single
...
if any(d.get(k) for k in ('independentChoice','finalAnswer','crossCheck')):
    sys.exit(1)
sys.exit(0)
"
}
...
process_one() {
    ...
    local prompt; prompt=$(sed 's/{{MODEL_IDENTIFIER}}/codex-gpt-5.5-xhigh/g' "$PROMPT_FILE")

    # codex exec:
    #   - stdin = batch JSON (appended as <stdin> block to the instructions)
    #   - prompt arg = system/instruction prompt (REVERIFY_PROMPT.md)
...
# Sanity: must have the key fields populated (schema matches REVERIFY_PROMPT.md)
if not (d.get('independentChoice') and d.get('finalAnswer') and d.get('crossCheck')):
    print('missing required fields', file=sys.stderr); sys.exit(1)
# Stamp uid (verifiedBy is already correct because the prompt {{MODEL_IDENTIFIER}}
# placeholder was substituted to 'codex-gpt-5.5-xhigh' before the codex call).
d['uid'] = '$uid'
