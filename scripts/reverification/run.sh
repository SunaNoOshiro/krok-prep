...
# Mirrors .tmp_rewrite/krok-file-2_enrich/run.sh — same JSON extraction +
# skip-if-done + xargs -P parallelism — but iterates over arbitrary uid'd
# batch files instead of fixed q### range.
#
# Usage (run from inside scripts/reverification/):
#   ./run.sh                    # all batches, JOBS=3
#   ./run.sh <uid_pattern>      # filter to matching uids (substring)
#   ./run.sh <pattern> <jobs>
#
