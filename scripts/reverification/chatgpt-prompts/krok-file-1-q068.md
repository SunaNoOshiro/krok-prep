...
## Питання (uid: `krok-file-1-q068`)

Пацієнта віком 32 роки шпиталізовано з глибокими опіками ділянки ліктьового суглоба. Протипоказів до обстеження амплітуди рухів немає. Протягом якого терміну треба провести обстеження амплітуди рухів ліктьового суглоба?

A. 48 годин з моменту отримання травми
B. 24 годин з моменту шпиталізації
C. 36 годин з моменту шпиталізації
D. 72 годин з моменту отримання травми
E. 24 годин з моменту отримання травми
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | D | 72 годин з моменту отримання травми | medium | У реабілітації опікових пацієнтів первинне обстеження амплітуди рухів суглобів, прилеглих до глибокого опіку, рекомендують виконати у вікно до 72 годин від моменту травми — до розвитку набряку та формування рубцевих конт… |
| `krok-file-8` | E | 72 годин з моменту отримання травми | — | — |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-1-q068.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-1-q068",
  "verifiedBy": "chatgpt",
  ...
  "perSourceVerdict": [
    ...
    {"file": "krok-file-8", "claimedKey": "e", "agrees": <true|false>, "reason": "<1-2 речення>"}
  ],
  "finalAnswer": {
    "key": "a|b|c|d|e",
    "text": "<verbatim>",
