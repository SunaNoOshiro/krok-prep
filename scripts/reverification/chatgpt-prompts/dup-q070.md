...
## Питання (uid: `dup-q070`)

Як називається вільний у просторі рух, що виконується дистальним сегментом кінцівки?

A. Зворотна вправа
B. Поступальна вправа
C. Функціюнальна вправа
D. Вирава з відкритим кінематичним ланцюгом
E. Вправа із закритим кінематичним ланцюгом
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | D | Вирава з відкритим кінематичним ланцюгом | high | За визначенням біомеханіки, відкритий кінематичний ланцюг — це ланка, у якій дистальний сегмент вільний і не зафіксований опорою (рух кисті чи стопи в просторі). Формулювання умови дослівно відповідає цьому визначенню. З… |
| `krok-file-3` | D | Вправа з відкритим кінематичним ланцюгом | high | Відкритий кінематичний ланцюг — це рух, при якому дистальний сегмент кінцівки вільний у просторі й не зафіксований опорою (наприклад, розгинання гомілки сидячи). У закритому ланцюгу дистальний сегмент фіксований до опори… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q070.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
