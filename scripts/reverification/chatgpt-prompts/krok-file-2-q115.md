...
## Питання (uid: `krok-file-2-q115`)

Фізичний терапевт вимірює амплітуду рухів у надп'ятково-гомілковому суглобі пацієнта. Навколо якої вісі відбувається згинання та розгинання стопи із загальним обсягом до 60-70°?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | B | Фронтальної | high | За стандартною українською й класичною анатомічною номенклатурою (Синельников, Ковешніков, Вовк) згинання та розгинання в будь-якому суглобі відбуваються навколо фронтальної (поперечної) осі, а саме рух виконується в саг… |
| `krok-file-3` | B | Фронтальної | high | Надп'ятково-гомілковий суглоб — блокоподібний одноосьовий; його єдина вісь руху проходить поперечно через блок таранної кістки у фронтальній площині. Обертання навколо неї дає рух у сагітальній площині — плантарфлексію (… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q115.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q115",
  "verifiedBy": "chatgpt",
