...
## Питання (uid: `krok-file-2-q112`)

Під час обстеження виявлено, що пацієнт не може здійснити підошвове згинання стопи та пальців, але при цьому він може стояти на п'ятках, але не може стояти на пальцях. Який нерв, найімовірніше, пошкоджено?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | C | Великогомілковий | high | Підошвове згинання стопи та пальців забезпечують gastrocnemius, soleus і довгі згиначі пальців — усі іннервуються великогомілковим нервом. Збережене стояння на п'ятках вказує на цілий малогомілковий нерв. Латеральний під… |
| `krok-file-3` | C | Великогомілковий | high | Підошвове згинання стопи та пальців забезпечують m. gastrocnemius, soleus, tibialis posterior, flexor digitorum/hallucis longus — усі іннервуються великогомілковим нервом. Збережена опора на п'ятах означає, що тильне зги… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q112.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q112",
  "verifiedBy": "chatgpt",
