...
## Питання (uid: `krok-file-2-q024`)

Пацієнт віком 73 роки хворіє на хворобу Паркінсона, акінетико-ригідну форму. Після тестування визначена IV стадія хвороби за модифікованою шкалою Хена та Яра. Якому рівню рухової активності відповідає ця стадія хвороби в пацієнта?

A. Істотне обмеження рухової активності, пацієнт не може самостійно пересуватися
B. Помірне обмеження рухової активності, легка залежність пацієнта від сторонньої допомоги в повсякденному житті
C. Легке обмеження рухової активності, пацієнт не потребує сторонньої допомоги
D. Істотне обмеження рухової активності, але залишається можливим самостійне пересування
E. Помірне обмеження рухової активності, пацієнт потребує істотної сторонньої допомоги в повсякденному житті
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | D | Істотне обмеження рухової активності, але залишається можливим самостійне пересування | high | IV стадія за модифікованою шкалою Хена-Яра канонічно визначається поєднанням тяжкої (істотної) інвалідизації зі збереженою здатністю до самостійного стояння й ходьби — це формулювання дослівно відтворює варіант (d). Варі… |
| `krok-file-3` | D | Істотне обмеження рухової активності, але залишається можливим самостійне пересування | high | За модифікованою шкалою Хена-Яра IV стадія описується як важка інвалідизація з вираженою постуральною нестабільністю, при якій пацієнт усе ще здатний стояти й самостійно ходити без сторонньої допомоги. Повна втрата самос… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q024.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q024",
  "verifiedBy": "chatgpt",
