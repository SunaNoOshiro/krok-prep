...
## Питання (uid: `krok-file-3-q046`)

Під час оцінки результатів проби Мартіне-Кушелевського виявилося, що в пацієнта збудливість пульсу більше 80%, час відновлення артеріальний тиск до навантаження – 120/80 мм рт. ст., а після навантаження – систолічний значно підвищився, діастолічний значно знизився (180/50 мм рт. ст.).
...
|------|------|-----------------|------------|------------------------|
| `krok-file-3` | A | Дистонічний | high | За класифікацією типів реакції на пробу Мартіне-Кушелевського гіпертонічний тип передбачає підвищення і систолічного, і діастолічного тиску. В умові ж САД різко зріс (до 180), а ДАД різко знизився (до 50) — це класична к… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-3-q046.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-3-q046",
  "verifiedBy": "chatgpt",
