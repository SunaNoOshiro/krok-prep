...
## Питання (uid: `krok-file-2-q107`)

Під час оцінювання рівня спастичності двоголового м'яза плеча за шкалою Ашворт фізичний терапевт відчуває опір м'яза протягом усього руху, але пасивний рух виконано в повній амплітуді. Якою кількістю балів за шкалою Ашворта можна оцінити спастичність м'яза в цьому разі?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | B | 1 | medium | У більшості українських джерел з підготовки до КРОК 2 1 бал за шкалою Ашворта визначається як легке підвищення тонусу з відчутним опором при пасивному русі за умови повного збереження амплітуди. Опис «опір протягом усьог… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q107.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q107",
  "verifiedBy": "chatgpt",
