...
## Питання (uid: `dup-q061`)

Після поранення в ділянці пахвової ямки пацієнт відчуває труднощі з розгинанням і приведенням плеча до тулуба, а також обертанням його досередини. Який м'яз ушкоджено в пацієнта?

A. Двоголовий м'яз плеча
B. Найширший м'яз спини
C. Дельтоподібний
D. Тритоголовий м'яз плеча
E. Великий круглий
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | B | Найширший м'яз спини | high | Тріада 'розгинання + приведення + внутрішня ротація плеча' — канонічний функціональний 'портрет' найширшого м'яза спини (m. latissimus dorsi). Великий круглий є функціональним синергістом, але саме найширший — потужний і… |
| `krok-file-3` | E | Великий круглий | high | Класична функціональна тріада плечового суглоба «розгинання + приведення + внутрішня ротація» належить великому круглому м'язу (та найширшому спини, якого немає серед варіантів). Локалізація рани в пахвовій ямці узгоджує… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q061.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
