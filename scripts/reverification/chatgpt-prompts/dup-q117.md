...
`cross_file_mismatch, medium_confidence`
...
## Питання (uid: `dup-q117`)

Яку стратегію потрібно вибрати для реалізації завдання із вдосконалення навичок пересідання на рівновисокі поверхні особою з наслідками хребетно- спинномозкової травми на рівні С7 (ASIA C)?

A. Пересідання з відпрацюванням належного рівня рівноваги тіла
B. Пересідання з частковою допомогою фізичного терапевта
C. Пересідання за допомогою ковзної поверхні
D. Пересідання з вербальною допомогою фізичного терапевта
E. Пересідання з повною допомогою фізичного терапевта
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | E | Пересідання з повною допомогою фізичного терапевта | medium | ASIA C означає рухово неповне ушкодження, де більше половини ключових м'язів нижче неврологічного рівня мають силу менше 3 балів. Навіть на рівні С7, де частково збережена функція трицепса, такий моторний дефіцит не дозв… |
| `krok-file-3` | C | Пересідання за допомогою ковзної дошки | high | При ХСМТ на рівні C7 ASIA C збережені розгиначі ліктя і плечовий пояс, але контроль тулуба й функція кисті слабкі. Стандартом пересідання на рівновисокі поверхні на цьому рівні є transfer board (ковзна дошка), яка забезп… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q117.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
