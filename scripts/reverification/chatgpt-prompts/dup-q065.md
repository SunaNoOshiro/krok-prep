...
## Питання (uid: `dup-q065`)

У постінсультного пацієнта з геміпарезом за результатами мануально- м'язового тестування сила двоголового м'яза стегна становить 1 бал. Якому кваліфікатору МКФ для функцій організму відповідає зазначений показник сили м'язів?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | A | 3 | high | Кваліфікатори МКФ для функцій організму — це шкала відсотка втрати: 0 (0–4%), 1 (5–24%), 2 (25–49%), 3 (50–95%), 4 (96–100%). MMT 1 бал означає лише слід скорочення без руху сегмента — це приблизно 80% дефіциту сили, що … |
| `krok-file-3` | C | 4 | high | Кваліфікатори МКФ для функцій організму ранжовані 0–4, де 4 — повне порушення (96–100% втрати функції). MMT 1 бал означає лиш слідове скорочення без активного руху проти гравітації або в полегшеному положенні, тобто факт… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q065.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
