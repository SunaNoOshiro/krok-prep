...
## Питання (uid: `krok-file-1-q047`)

Із якою метою застосовують валик під стегна та нижню частину живота, виконуючи позиціонування лежачи на животі?

A. Для усунення лордозу поперекового відділу хребта
B. Для тренування м'язів живота
C. Для збільшення черевного тиску
D. Для зменшення черевного тиску
E. Для зменшення напруження м'язів плечового пояса та ротаторів
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | A | Для усунення лордозу поперекового відділу хребта | high | Стандарт позиціонування в положенні лежачи на животі: валик під тазом і нижньою частиною живота піднімає таз, переводить поперек у нейтральне положення й нейтралізує природний поперековий лордоз, який у цій позі гіперекс… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-1-q047.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-1-q047",
  "verifiedBy": "chatgpt",
  ...
  "perSourceVerdict": [
    {"file": "krok-file-1", "claimedKey": "a", "agrees": <true|false>, "reason": "<1-2 речення>"}
  ],
  "finalAnswer": {
    "key": "a|b|c|d|e",
    "text": "<verbatim>",
