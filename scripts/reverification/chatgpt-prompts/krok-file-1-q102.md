...
## Питання (uid: `krok-file-1-q102`)

Укажіть оптимальний термін для досягнення однієї короткотривалої функціональної цілі під час реабілітації пацієнта в стаціонарі.
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | B | 5-7 днів | medium | У вітчизняних методичних матеріалах з реабілітаційного менеджменту короткотривалі (short-term) функціональні цілі в стаціонарі планують на один тиждень — близько 5-7 днів, що збігається з циклом командної переоцінки. Це … |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-1-q102.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-1-q102",
  "verifiedBy": "chatgpt",
  ...
  "perSourceVerdict": [
    {"file": "krok-file-1", "claimedKey": "b", "agrees": <true|false>, "reason": "<1-2 речення>"}
  ],
  "finalAnswer": {
    "key": "a|b|c|d|e",
    "text": "<verbatim>",
