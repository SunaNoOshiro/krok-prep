...
## Питання (uid: `dup-q134`)

Фізичний терапевт проводить мануальне м'язове тестування привідних м'язів стегна. Який принцип є основним правилом тестування та оцінювання м'язової сили?

A. Напруження та розслаблення
B. Напруження та подолання
C. —
D. Розслаблення та напруження
E. Розслаблення та подолання
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | A | Напруження та розслаблення | medium | Принцип ММТ описує цикл роботи самого м'яза під час проби: пацієнт за командою активно скорочує м'яз (напруження), а потім повертає його у стан спокою (розслаблення). Саме ця парна фаза дозволяє грейдувати силу від 0 до … |
| `krok-file-3` | A | Напруження та подолання | high | Класичне мануальне м'язове тестування (ММТ) за Daniels & Worthingham / Kendall — це break-test: пацієнт виконує ізометричне напруження м'яза в тестовому положенні, а терапевт прикладає протитиск, намагаючись подолати цей… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q134.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
