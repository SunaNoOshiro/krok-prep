...
## Питання (uid: `krok-file-1-q074`)

Для ліквідації або зменшення яких контрактур треба використовувати постізометричну релаксацію (ПІР)?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | C | Міогенних | high | Постізометрична релаксація (за К. Левітом) — нейром'язова техніка: після ізометричного скорочення м'яза вмикається пост-ізометричне гальмування мотонейронів і рефлекс Гольджі, м'яз розслаблюється й піддається розтягненню… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-1-q074.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-1-q074",
  "verifiedBy": "chatgpt",
