...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | E | Ротова | high | Активна робота нижньої щелепи, язика, м'якого й твердого піднебіння та губ — це класичний опис ротової (oral) фази ковтання, під час якої формується й просувається болюс до зіву. Стравохідний рівень (зокрема будь-яка «ез… |
| `krok-file-3` | D | Ротова | high | Стандартна модель ковтання виділяє ротову (оральну), фарингеальну та езофагеальну фази; нижня щелепа, язик, тверде й м'яке піднебіння та губи задіяні лише в ротовій фазі — там болюс формується та просувається до глотки. … |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q095.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q095",
  "verifiedBy": "chatgpt",
