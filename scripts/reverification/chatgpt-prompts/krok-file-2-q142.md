...
## Питання (uid: `krok-file-2-q142`)

Чому на початку реабілітації травм плечового суглоба надається перевага вправам у закритому кінематичному ланцюзі?

A. Ізольована робота певних м'язових груп
B. Відтворення певних схем рухів
C. Залучення в роботу м'язів-антагоністів та синергістів
D. Менша швидкість руху, але більша стабільність
E. Менша стимуляція пропріоцептивної чутливості
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | C | Залучення в роботу м'язів-антагоністів та синергістів | high | Закритий кінематичний ланцюг за визначенням передбачає фіксацію дистального сегмента й осьове навантаження, що провокує ко-активацію антагоністів і синергістів — саме це створює стабільність суглоба й знижує зсувні сили … |
| `krok-file-3` | C | Залучення в роботу м'язів-антагоністів та синергістів | high | Вправи у закритому кінематичному ланцюзі через фіксацію дистальної ланки змушують м'язи навколо суглоба працювати спільно (агоністи + антагоністи + синергісти), що дає компресію й динамічну стабілізацію — критично для не… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q142.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q142",
  "verifiedBy": "chatgpt",
