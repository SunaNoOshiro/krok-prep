...
## Питання (uid: `dup-q092`)

Яку концептуальну модель практики використовує у своїй професійній діяльності ерготерапевт, коли аналізує заняттєву активність та важливі для особи заняття з метою їх покращення, враховуючи вплив середовища та наратив самої особи?

A. Канадська модель виконання заняття та залучення (CMOP-E)
B. Модель «Особа-середовище-заняття-виконання» (PEOP)
C. Модель процесу ерготерапевтичного втручання (OTIPM)
D. Модель заняттєвої активності людини (MOHO)
E. Модель KAWA «Річка»
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | B | Модель «Особа-середовище-заняття-виконання» (PEOP) | medium | PEOP (Christiansen, Baum, Bass) — єдина з варіантів концептуальна модель, що структурно поєднує всі чотири елементи з умови (особа, середовище, заняття, виконання) і канонічно використовує наратив клієнта як інструмент о… |
| `krok-file-3` | A | Канадська модель виконання заняття та залучення (CMOP-E) | high | CMOP-E — канонічна клієнт-центрована концептуальна модель, що структурно поєднує тріаду 'людина-заняття-середовище' з акцентом на клієнтське визначення важливих занять (через супровідний інструмент COPM) і врахуванням ко… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q092.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
