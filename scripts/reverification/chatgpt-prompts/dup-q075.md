...
## Питання (uid: `dup-q075`)

У пацієнта віком 48 років — лівобічний гонартроз 2 ступеня, пошкодження медіального меніска лівого колінного суглоба. Виражений больовий синдром. Який метод обстеження дозволить визначити обсяг активних рухів у пошкодженому суглобі?

A. Комп'ютерна томографія
B. Мануально-м'язове тестування
C. Рентгенографія
D. Гоніометрія (згідно з позначкою)
E. Пальпація
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | D | Гоніометрія (згідно з позначкою) | high | Гоніометрія — стандартний інструментальний метод вимірювання обсягу активних і пасивних рухів у суглобі в градусах. Інші варіанти оцінюють інші параметри: КТ і рентген — структуру кістки, ММТ — м'язову силу, пальпація — … |
| `krok-file-3` | D | Гоніометрія | high | Обсяг рухів у суглобі (range of motion) кількісно вимірюється кутоміром — гоніометром. Це базовий інструмент фізичного терапевта для оцінки амплітуди як активних, так і пасивних рухів. Інші перелічені методи оцінюють сил… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q075.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
