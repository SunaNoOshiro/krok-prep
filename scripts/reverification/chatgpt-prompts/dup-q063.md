...
## Питання (uid: `dup-q063`)

Під час вибору допоміжних засобів пересування орієнтуються на величину кута в ліктьовому суглобі. Який кут згинання в ліктьовому суглобі рекомендований при виборі тростини (ходунків, милиць, паралельних брусів)?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | B | 25-30° | high | Стандартним ергономічним орієнтиром при підборі тростини, милиць, ходунків і паралельних брусів є згинання в ліктьовому суглобі ≈ 25–30° при опорі на рукоятку, розташовану на рівні великого вертлюга стегна (або зап'ястка… |
| `krok-file-3` | A | 15-20° | high | У навчальній літературі з фізичної терапії висота допоміжного засобу пересування підбирається так, щоб рукоятка була на рівні шилоподібного відростка зап'ястка при вільно опущеній руці, що забезпечує згинання в ліктьовом… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q063.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
