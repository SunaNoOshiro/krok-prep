...
## Питання (uid: `krok-file-1-q049`)

Чоловіку віком 54 роки встановлено діагноз: ішемічна хвороба серця, інфаркт міокарда, ІІІ ФК. Пацієнт проходить постстаціонарну фізичну терапію в кардіологічному санаторії. Йому показані аеробні фізичні навантаження. Яку форму та дозу аеробного навантаження має призначити фізичний терапевт відповідно до функціонального класу на початку проведення санаторного курсу кардіореабілітації?

A. Дозована ходьба в повільному темпі (70-80 кроків/хв) із зупинками через кожні 100- 200 м
B. Дозована ходьба в темпі 90-100 кроків/хв з короткочасним прискоренням по 2-3 хв у темпі 120-30 кроків/хв
C. Дозована ходьба в помірному темпі (110 кроків/хв) з короткочасним прискоренням по 3-5 хв у темпі 120-30 кроків/хв
D. Дозована ходьба в середньому темпі (90 кроків/хв)
E. Біг у помірному темпі
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | D | Дозована ходьба в середньому темпі (90 кроків/хв) | medium | За класифікацією Аронова для дозованої ходьби в санаторії ФК III відповідає темп близько 80-90 кроків/хв без прискорень і без планових зупинок. Варіант A відображає лікарняний режим або ФК IV, B — це ФК II, C — ФК I, E в… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-1-q049.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-1-q049",
  "verifiedBy": "chatgpt",
