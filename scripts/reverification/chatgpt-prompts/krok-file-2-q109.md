...
## Питання (uid: `krok-file-2-q109`)

Чим зумовлений мінімальний час кардіотренування незалежно від рівня здоров'я?

A. Функціональним станом дихальної та серцево-судинної систем
B. Адаптацією дихальної та серцево-судинної систем до фізичного навантаження
C. Зростанням інтенсивності обміну речовин під час кардіотренування
D. Поступовим утворенням достатньої кількості енергії для роботи на витривалість
E. Дотриманням загальних умов застосування аеробних терапевтичних вправ у фізичній терапії
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | C | Зростанням інтенсивності обміну речовин під час кардіотренування | medium | Мінімальний час кардіотренування (зазвичай 20–30 хв у цільовій зоні) фізіологічно зумовлений часом, потрібним для виходу обміну речовин на стійкий аеробний робочий рівень. Варіант (d) описує наслідок цього процесу (енерг… |
| `krok-file-3` | C | Зростанням інтенсивності обміну речовин під час кардіотренування | medium | Мінімальна тривалість аеробного навантаження визначається часом виходу метаболізму на стійкий аеробний рівень — це універсальний фізіологічний процес, що відповідає варіанту (c). Варіант (d) близький за змістом, але опис… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q109.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q109",
  "verifiedBy": "chatgpt",
