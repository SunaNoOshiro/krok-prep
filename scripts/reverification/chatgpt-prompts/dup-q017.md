...
## Питання (uid: `dup-q017`)

Укажіть оптимальне антиконтрактурне положення у разі опіку стегна.

A. Нейтральне положення під кутом відведення 45^o
B. -
C. Нейтральне положення під кутом відведення 20^o
D. Нейтральне положення під кутом відведення 30^o
E. Нейтральне положення під кутом відведення 60^o
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | C | Нейтральне положення під кутом відведення 20^o | high | При опіку стегна природна тенденція — формування адукційно-згинальної контрактури кульшового суглоба. Стандартом профілактичного укладання є нейтральна ротація з помірним відведенням близько 15–20°. Менший кут не протиді… |
| `krok-file-8` | A | Нейтральне положення під кутом відведення 20° | — | — |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q017.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
...
```json
{
  ...
  "perSourceVerdict": [
    {"file": "krok-file-1", "claimedKey": "c", "agrees": <true|false>, "reason": "<1-2 речення>"},
    {"file": "krok-file-8", "claimedKey": "a", "agrees": <true|false>, "reason": "<1-2 речення>"}
  ],
  "finalAnswer": {
    "key": "a|b|c|d|e",
    "text": "<verbatim>",
