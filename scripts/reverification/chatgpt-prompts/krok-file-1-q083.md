...
## Питання (uid: `krok-file-1-q083`)

Фізичний терапевт проводить 6-хвилинний тест ходьби чоловіку з порушенням діяльності ССС. Під час тестування пацієнт зупинився та сів на стілець для відпочинку. Які подальші дії фізичного терапевта?

A. Тест закінчується, фіксується час та підраховується пройдена відстань
B. Відлік часу зупиняється, у протоколі фіксується час відпочинку, після відпочинку пацієнт продовжує тестування
C. Відлік часу не зупиняється, фізичний терапевт пропонує продовжити тестування за допомогою допоміжного засобу, у протоколі фіксується час відпочинку
D. Відлік часу зупиняється, фізичний терапевт оцінює життєво важливі показники, у протоколі фіксуються час відпочинку, після відпочинку пацієнт продовжує тестування
E. Відлік часу не зупиняється, у протоколі фіксується час відпочинку, після відпочинку пацієнт продовжує тестування
...
|------|------|-----------------|------------|------------------------|
| `krok-file-1` | E | Відлік часу не зупиняється, у протоколі фіксується час відпочинку, після відпочинку пацієнт продовжує тестування | high | Стандарт ATS для 6-хвилинного тесту ходьби чітко вказує: якщо пацієнт потребує відпочинку, секундомір не зупиняють, пацієнта заохочують продовжити, коли він зможе; у протоколі фіксують факт і час паузи. Дострокове припин… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-1-q083.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-1-q083",
  "verifiedBy": "chatgpt",
