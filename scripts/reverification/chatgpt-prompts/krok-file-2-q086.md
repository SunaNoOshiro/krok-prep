...
## Питання (uid: `krok-file-2-q086`)

Пацієнт скаржиться, що не може розігнути верхню кінцівку в ліктьовому, променево-зап'ястковому та п'ястково-фалангових суглобах, відвести великий палець, а також здійснити супінацію передпліччя та кисті. Який нерв, найімовірніше, пошкоджено?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | C | Променевий | high | Розгинання у ліктьовому, променевозап'ястковому та п'ястково-фалангових суглобах виконують triceps brachii та екстензори передпліччя; відведення великого пальця у площині, перпендикулярній до долоні, забезпечує abductor … |
| `krok-file-3` | C | Променевий | high | Випадіння розгинання у лікті (трицепс), кисті та пальців («звисаюча кисть»), відведення великого пальця та супінації — це класична картина ураження променевого нерва на високому рівні (плече/пахвова западина). Супінатор … |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q086.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q086",
  "verifiedBy": "chatgpt",
