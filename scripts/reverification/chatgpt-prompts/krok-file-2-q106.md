...
## Питання (uid: `krok-file-2-q106`)

Пацієнт віком 45 років скаржиться фізичному терапевту на біль у спині та обмеження рухів у грудному відділі хребта. В анамнезі: травма спини 5 років тому. Для оцінки рухливості хребта фізичний терапевт вирішив провести тест Отта. Під час виконання тесту фізичний терапевт відзначив нормальну рухливість хребта. На скільки сантиметрів збільшується відстань під час виконання тесту Отта при нормальній рухливості хребта?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | E | 3,5-5 | high | Тест Отта вимірює рухливість грудного відділу хребта: від остистого відростка C7 відмірюють 30 см донизу, після чого пацієнт нахиляється вперед. У здорової людини ця відстань збільшується на 3,5-5 см (близько 4 см). Це с… |
| `krok-file-3` | E | 3,5-5 | medium | Тест Отта оцінює рухливість грудного відділу: позначки на C7 і 30 см нижче, при флексії відстань збільшується. В україномовних джерелах підготовки до КРОК нормою найчастіше називають приріст 3,5–5 см (міжнародні джерела … |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q106.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q106",
  "verifiedBy": "chatgpt",
