...
## Питання (uid: `dup-q069`)

Пацієнт віком 56 років проходить ранній етап реабілітації після тотального ендопротезування кульшового суглоба. Яку модель ходьби необхідно застосовувати в цьому разі?

A. Приточкову з милицями
B. Двоточкову з милицями
C. Чотириточкову з милицями
D. З шестипроменевими палицями
E. З ходунками
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | A | Приточкову з милицями | high | У ранньому періоді після тотального ендопротезування кульшового суглоба стандарт — приточкова (трьохточкова) хода з милицями: одночасний винос обох милиць та оперованої ноги дозволяє максимально розвантажити ендопротез у… |
| `krok-file-3` | A | Триточкову з милицями | high | На ранньому етапі після тотального ендопротезування кульшового суглоба показана хода з двома милицями за триточковою схемою: дві милиці й оперована нога переставляються одночасно, здорова нога окремо. Це забезпечує дозов… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q069.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
