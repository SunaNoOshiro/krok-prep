...
## Питання (uid: `dup-q066`)

Постінсультному пацієнту віком 60 років діагностовано: спастичний геміпарез, спастичність м'язів-згиначів верхньої кінцівки становить 3-4 бали за шкалою Ашворта, больовий синдром. Яке втручання ерготерапії є оптимальним для пацієнта з урахуванням ступеня спастичності?

A. Вібраційний масаж
B. Ботулінотерапія
C. Ортезування
D. Теплові процедури
E. Тривале статичне розтягнення
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | E | Тривале статичне розтягнення | high | Питання прямо обмежує відповідь рамкою «втручання ерготерапії», що автоматично виключає ботулінотерапію як медичну маніпуляцію. При спастичності Ашворт 3-4 з больовим синдромом тривале низькоінтенсивне статичне розтягнен… |
| `krok-file-3` | E | Тривале статичне розтягнення м'язів | high | Питання прямо обмежує відповідь рамкою ерготерапії, що виключає ботулінотерапію як медичну ін'єкційну процедуру. Вібрація та тепло при Ашворті 3-4 із сенсорними розладами після інсульту або підсилюють тонус, або дають ко… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q066.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
