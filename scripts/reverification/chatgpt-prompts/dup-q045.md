...
## Питання (uid: `dup-q045`)

Пацієнт за призначенням фізичного терапевта виконує вправу «планка». Укажіть характерну особливість статичної роботи м'язів у цьому разі.

A. Незначне напруження
B. Значне скорочення м'яза
C. Відносно пізнє настання втоми
D. Наявне переміщення
E. Наявне напруження
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | E | Наявне напруження | high | Планка — класична ізометрична (статична) вправа. Ізометричний режим визначається наявністю м'язової тяги без зміни довжини м'яза та без переміщення в суглобі. Інші варіанти або описують ізотонічну роботу (скорочення, пер… |
| `krok-file-3` | D | Значне напруження | high | Планка — класичний приклад ізометричної (статичної) роботи м'язів кора, плечового пояса та стегон проти сили тяжіння. Для такого режиму характерні розвиток значного напруження без зміни довжини м'яза і без переміщення ла… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/dup-q045.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:
