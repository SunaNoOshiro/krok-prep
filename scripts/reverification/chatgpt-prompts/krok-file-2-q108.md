...
## Питання (uid: `krok-file-2-q108`)

Під час дослідження моделі ходи в пацієнта спостерігається втрата природного замаху руки та перетягування ураженої ноги півколом. Який це тип аномальної ходи?
...
|------|------|-----------------|------------|------------------------|
| `krok-file-2` | A | Геміплегічний | high | Поєднання двох ознак — відсутність фізіологічного замаху ураженої руки та перетягування ноги півколом (циркумдукція) — є хрестоматійним описом геміплегічної ходи Wernicke-Mann після інсульту чи іншого пірамідного ураженн… |
| `krok-file-3` | A | Геміплегічний | high | Описаний патерн — класична хода Верніке-Манна: однобічна втрата фізіологічного замаху руки при її флексорній позі та циркумдукція ноги через спастичну екстензію в коліні й еквінусну стопу. Це канонічна геміплегічна (гемі… |
...
В кінці своєї відповіді **обов'язково** виведи структурований JSON у кодовому блоці точно такого формату — користувач скопіює його у файл `chatgpt-prompts/responses/krok-file-2-q108.json`, а скрипт `build_disputed.py` потім автоматично змерджить у `disputed-questions.json`:

```json
{
  "uid": "krok-file-2-q108",
  "verifiedBy": "chatgpt",
